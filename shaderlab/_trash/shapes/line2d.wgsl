...
VertexOutput vert(VertexInput v) {
    ...
    switch (alignment) {
        case ALIGNMENT_FLAT:
            let local_z = normalize(vec3<f32>(UNITY_MATRIX_M[0].z, UNITY_MATRIX_M[1].z, UNITY_MATRIX_M[2].z));
            normal = cross(tangent, local_z);
        break;
        case ALIGNMENT_BILLBOARD:
            let cam_forward = -DirectionToNearPlanePos(vertOrigin);
            normal = normalize(cross(tangent,camForward));
        break;
    }

    let scaleMode = UNITY_ACCESS_INSTANCED_PROP(Props, _ScaleMode);
    let uniformScale = GetUniformScale();
	let scale_thickness = scaleMode == SCALE_MODE_UNIFORM ? uniformScale : 1;
	let scale_dashes = scaleMode == SCALE_MODE_UNIFORM ? uniformScale : 1;
	let scale_spacing = uniformScale;



	let thickness = UNITY_ACCESS_INSTANCED_PROP(Props, _Thickness) * scaleThickness;
...
FRAG_OUTPUT_V4 frag( VertexOutput i ) : SV_Target {
	...
	let colorStart = UNITY_ACCESS_INSTANCED_PROP(Props, _Color);
    let colorEnd = UNITY_ACCESS_INSTANCED_PROP(Props, _ColorEnd);
	let shape_color = lerp(colorStart, colorEnd, saturate( i.IP_tColor ) );
