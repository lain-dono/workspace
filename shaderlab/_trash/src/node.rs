...
use iced_native::widget::{Column, Container, Row, Rule, Space};
...
impl NodeWidget {
    ...
    pub fn or_default<T: crate::builder::NodeBuilder>(
        &self,
        index: usize,
        function: &mut FunctionBuilder,
        map: impl FnOnce([f64; 4]) -> T,
    ) -> Option<Handle<Expression>> {
        if let Some(output) = self.inputs_remote[index] {
            function.node(output)
        } else {
            map(self.defaults[index].value).expr(function, Default::default())
        }
    }
