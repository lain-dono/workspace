...
impl Node for Input {
    fn expr(&self, function: &mut FunctionBuilder, output: Output) -> Option<Handle<Expression>> {
        match self.ty {
            Type::Scalar => self.node.defaults[0].value[0].expr(function, output),
            Type::Vector2 => [
                self.node.defaults[0].value[0],
                self.node.defaults[1].value[0],
            ]
            .expr(function, output),
            Type::Vector3 => [
                self.node.defaults[0].value[0],
                self.node.defaults[1].value[0],
                self.node.defaults[2].value[0],
            ]
            .expr(function, output),
            Type::Vector4 => [
                self.node.defaults[0].value[0],
    ...
    fn inputs(&self) -> &[port::State] {
        &self.node.ports.inputs
    }
    fn outputs(&self) -> &[port::State] {
        &self.node.ports.outputs
    }
    fn update(&mut self, event: Event) {
        self.node.update(event)
    }
    fn view(&mut self, node: NodeId) -> NodeElement {
        self.node.view(node, None)
    }
}
