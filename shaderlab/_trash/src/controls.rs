    ...
    pub fn new() -> Self {
        Self {
            workspace: workspace::State::with_nodes({
                ...
                nodes.add(Point::new(0.0, 100.0), input::Input::new(Type::Vector4));
                nodes.add(Point::new(0.0, 300.0), input::Color::default());
                nodes.add(Point::new(300.0, 20.0), input::Triangle::default());
                nodes.add(Point::new(400.0, 20.0), input::Fullscreen::default());
                nodes.add(Point::new(400.0, 200.0), math::Add::default());
...
fn node_list(scrollable: Scrollable<Message, Renderer>) -> Scrollable<Message, Renderer> {
    ...
    fn def<T: Default + Node>() -> Box<dyn Node> {
        Box::new(T::default())
    }
    ...
    scrollable
        ...
        //.row(&["bool", "color", "f32"])
        //.row(&["vec2", "vec3", "vec4"])
        .rowx(&[("triangle", def::<input::Triangle>)])
        .rowx(&[("fullscreen", def::<input::Fullscreen>)])
        .rowx(&[
            ("vec1", || Box::new(input::Input::new(Type::Scalar))),
            ("vec2", || Box::new(input::Input::new(Type::Vector2))),
            ("vec3", || Box::new(input::Input::new(Type::Vector3))),
            ("vec4", || Box::new(input::Input::new(Type::Vector4))),
        ])
        .rowx(&[("color", def::<input::Color>)])
