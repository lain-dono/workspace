...
use super::reflect_editor::{get_component_reflect, get_resource_reflect};
use bevy::ecs::{prelude::*, world::unsafe_world_cell::UnsafeWorldCell};
...
pub(crate) fn guess_entity_name_restricted(
    world: &mut super::restricted_world_view::RestrictedWorldView<'_>,
    entity: Entity,
) -> String {
    if let Some(cell) = world.world().get_entity(entity) {
        ...
        let name = unsafe { name_for_entity(world.world(), entity).unwrap_or("Entity") };
        format!("{name} ({entity:?})")
    } else {
        format!("Entity {} (inexistent)", entity.index())
    }
}
*/
...
impl<'w> RestrictedWorldView<'w> {
    ...
    unsafe fn get_resource_unchecked_mut<R: Resource>(&self) -> Result<Mut<'_, R>, Error> {
        ...
        if self.allows_access_to_resource(type_id) {
            ...
            self.world().get_resource_mut::<R>().ok_or(err)
        } else {
            Err(Error::NoAccessToResource(type_id))
        }
    }
    ...
    pub fn get_resource_reflect_mut_by_id(
        ...
        if self.allows_access_to_resource(type_id) {
            unsafe { get_resource_reflect(self.world(), type_id, registry) }
        } else {
            Err(Error::NoAccessToResource(type_id))
        }
    }
    ...
    pub fn get_entity_component_reflect(
        ...
        if self.allows_access_to_component((entity, type_id)) {
            unsafe { get_component_reflect(self.world(), entity, type_id, registry) }
        } else {
            Err(Error::NoAccessToComponent(entity, type_id))
        }
    }
    ...
    pub(crate) unsafe fn get_entity_component_reflect_unchecked(
        ...
        if self.allows_access_to_component((entity, type_id)) {
            get_component_reflect(self.world(), entity, type_id, registry)
        } else {
            Err(Error::NoAccessToComponent(entity, type_id))
        }
    }
