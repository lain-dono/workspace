use super::parser::{Item, Parser, Style};
use eframe::egui::{
    Align, Align2, Hyperlink, Layout, Response, RichText, Sense, Separator, Shape, TextStyle, Ui,
    vec2,
};

/// Parse and display a VERY simple and small subset of Markdown.
pub fn easy_mark(ui: &mut Ui, easy_mark: &str) {
    mark_it(ui, Parser::new(easy_mark));
}

pub fn mark_it<'em>(ui: &mut Ui, items: impl Iterator<Item = Item<'em>>) {
    let initial_size = vec2(
        ui.available_width(),
        ui.spacing().interact_size.y, // Assume there will be
    );

    let layout = Layout::left_to_right(Align::BOTTOM).with_main_wrap(true);

    ui.allocate_ui_with_layout(initial_size, layout, |ui| {
        ui.spacing_mut().item_spacing.x = 0.0;
        ui.set_row_height(ui.text_style_height(&TextStyle::Body));

        for item in items {
            item_ui(ui, item);
        }
    });
}

pub fn item_ui(ui: &mut Ui, item: Item<'_>) {
    let row_height = ui.text_style_height(&TextStyle::Body);
    let one_indent = row_height / 2.0;

    match item {
        Item::Newline => {
            // ui.label("\n"); // too much spacing (paragraph spacing)
            ui.allocate_exact_size(vec2(0.0, row_height), Sense::hover()); // make sure we take up some height
            ui.end_row();
            ui.set_row_height(row_height);
        }
        Item::Text(style, text) => {
            let label = rich_text_from_style(text, &style);
            if style.small && !style.raised {
                ui.with_layout(Layout::left_to_right(Align::BOTTOM), |ui| {
                    ui.set_min_height(row_height);
                    ui.label(label);
                });
            } else {
                ui.label(label);
            }
        }
        Item::Hyperlink(style, text, url) => {
            let label = rich_text_from_style(text, &style);
            if style.small && !style.raised {
                ui.with_layout(Layout::left_to_right(Align::BOTTOM), |ui| {
                    ui.set_height(row_height);
                    ui.add(Hyperlink::from_label_and_url(label, url));
                });
            } else {
                ui.add(Hyperlink::from_label_and_url(label, url));
            }
        }

        Item::Separator => {
            ui.add(Separator::default().horizontal());
        }
        Item::Indentation(indent) => {
            let indent = indent as f32 * one_indent;
            ui.allocate_exact_size(vec2(indent, row_height), Sense::hover());
        }
        Item::QuoteIndent => {
            let size = vec2(2.0 * one_indent, row_height);
            let (rect, _) = ui.allocate_exact_size(size, Sense::hover());
            let rect = rect.expand2(ui.style().spacing.item_spacing * 0.5);
            ui.painter().line_segment(
                [rect.center_top(), rect.center_bottom()],
                (1.0, ui.visuals().weak_text_color()),
            );
        }
        Item::BulletPoint => {
            ui.allocate_exact_size(vec2(one_indent, row_height), Sense::hover());
            bullet_point(ui, one_indent);
            ui.allocate_exact_size(vec2(one_indent, row_height), Sense::hover());
        }
        Item::NumberedPoint(number) => {
            let width = 3.0 * one_indent;
            numbered_point(ui, width, number);
            ui.allocate_exact_size(vec2(one_indent, row_height), Sense::hover());
        }
        Item::CodeBlock(_language, code) => {
            let where_to_put_background = ui.painter().add(Shape::Noop);
            let mut rect = ui.monospace(code).rect;
            rect = rect.expand(1.0); // looks better
            rect.max.x = ui.max_rect().max.x;
            let code_bg_color = ui.visuals().code_bg_color;
            let shape = Shape::rect_filled(rect, 1.0, code_bg_color);
            ui.painter().set(where_to_put_background, shape);
        }
    }
}

fn rich_text_from_style(text: &str, style: &Style) -> RichText {
    fn maybe(rich_text: RichText, is: bool, f: impl FnOnce(RichText) -> RichText) -> RichText {
        if is { f(rich_text) } else { rich_text }
    }

    let Style {
        heading,
        quoted,
        code,
        strong,
        underline,
        strikethrough,
        italics,
        small,
        raised,
    } = *style;

    let small = small || raised; // Raised text is also smaller

    let mut rich_text = RichText::new(text);
    rich_text = maybe(rich_text, heading && !small, |rt| rt.heading().strong());
    rich_text = maybe(rich_text, small && !heading, RichText::small);
    rich_text = maybe(rich_text, code, RichText::code);
    rich_text = maybe(rich_text, strong, RichText::strong);
    rich_text = maybe(rich_text, quoted && !strong, RichText::weak);
    rich_text = maybe(rich_text, underline, RichText::underline);
    rich_text = maybe(rich_text, strikethrough, RichText::strikethrough);
    rich_text = maybe(rich_text, italics, RichText::italics);
    rich_text = maybe(rich_text, raised, RichText::raised);
    rich_text
}

fn bullet_point(ui: &mut Ui, width: f32) -> Response {
    let row_height = ui.text_style_height(&TextStyle::Body);
    let (rect, response) = ui.allocate_exact_size(vec2(width, row_height), Sense::hover());
    ui.painter().circle_filled(
        rect.center(),
        rect.height() / 8.0,
        ui.visuals().strong_text_color(),
    );
    response
}

fn numbered_point(ui: &mut Ui, width: f32, number: &str) -> Response {
    let font_id = TextStyle::Body.resolve(ui.style());
    let row_height = ui.fonts_mut(|f| f.row_height(&font_id));
    let (rect, response) = ui.allocate_exact_size(vec2(width, row_height), Sense::hover());
    ui.painter().text(
        rect.right_center(),
        Align2::RIGHT_CENTER,
        format!("{number}."),
        font_id,
        ui.visuals().strong_text_color(),
    );
    response
}
