...
fn read_error(p: &Path, e: io::Error) -> RotoReport {
    RotoReport {
...
impl SourceFile {
    ...
    fn read_internal(path: &Path) -> io::Result<Self> {
        let file_name = path.file_name().ok_or(io::Error::other("invalid path"))?;
        let module_name = if file_name == "mod.roto" {
            ...
            path.file_stem().ok_or(io::Error::other("invalid path"))?
        }
        .to_string_lossy()
        .to_string();
...
impl FileTree {
    ...
    #[must_use]
    pub fn test_file(file: &str, source: &str, location_offset: usize) -> Self {
        Self {
    ...
    #[must_use]
    pub fn file_spec(file_spec: FileSpec) -> Self {
        fn inner(parent: usize, files: &mut Vec<SourceFile>, file_spec: FileSpec) {
            match file_spec {
                FileSpec::File(file) => {
                    let idx = files.len();
                    files.push(file);
    ...
    pub fn directory(root: &Path) -> Result<Self, RotoReport> {
    ...
    fn find_files(&mut self, parent_id: usize, path: &Path) -> Result<(), RotoReport> {
        for entry in std::fs::read_dir(path).map_err(|e| read_error(path, e))? {
            ...
            let ident = path
                .file_stem()
                .ok_or_else(|| read_error(&path, io::Error::other("invalid path")))?
                .to_str()
                .ok_or_else(|| {
                    read_error(
                        &path,
                        io::Error::other("file name is not a valid Roto identifier"),
                    )
                })?;
    ...
    fn process_subdir(&mut self, parent_id: usize, path: &Path) -> Result<(), RotoReport> {
