use crate::{
    ...
    print::TypeDisplay,
    runtime::Runtime,
    types::{DeclKind, ScopeRef, Type, TypeOrStub, ValueKind},
};
...
impl Runtime {
    ...
    fn print_ty(&self, ty: &impl TypeDisplay) -> String {
        ty.display(&self.type_checker.types).to_string()
    }
    ...
    fn get_items(&self, scope: ScopeRef) -> Vec<DocItem> {
        ...
        let graph = self.type_checker.scopes();
        let decs = graph.declarations_in(scope);

        for dec in decs {
            match &dec.kind {
                DeclKind::Value(value_kind, ty) => match value_kind {
                ...
                DeclKind::Type(t) => {
                    ...
                    let scope = self
                        .type_checker
                        .scope_of(dec.name.scope, dec.name.ident)
                        .unwrap();
                ...
                DeclKind::Variant(_variant) => {
                    // Skip for now
                }
                DeclKind::TypeParam(_name) => {
    ...
    fn split_items(items: &[DocItem]) -> (Vec<&DocFn>, Vec<&DocConst>, Vec<&DocTy>, Vec<&DocMod>) {
        let mut fs = Vec::new();
        let mut cs = Vec::new();
        let mut ts = Vec::new();
        let mut ms = Vec::new();
