use roto::pipeline::RotoReport;
use roto::{FileSpec, FileTree, Library};
...
fn typecheck(loaded: FileTree) -> Result<(), RotoReport> {
    typecheck_with_runtime(loaded, Runtime::builtin())
}

#[track_caller]
fn typecheck_with_runtime(loaded: FileTree, rt: Runtime) -> Result<(), RotoReport> {
    let res = loaded.parse().unwrap();
...
fn use_globals() {
    ...
    let s = src!("filtermap main() { accept BLACKHOLE }");

    let rt = Runtime::from_lib({
        let mut lib = roto::Library::default();
        lib.define_clone_ty::<Val<Community>>("Community", "");
        lib.define_const(
            "BLACKHOLE",
