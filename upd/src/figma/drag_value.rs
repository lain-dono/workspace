...
use bevy_egui::egui::{
    Button, CursorIcon, Id, Key, Memory, Modifiers, NumExt as _, Response, RichText, Sense,
...
fn gained_focus(memory: &Memory, id: Id) -> bool {
    !memory.had_focus_last_frame(id) && memory.has_focus(id)
}
fn lost_focus(memory: &Memory, id: Id) -> bool {
    memory.had_focus_last_frame(id) && !memory.has_focus(id)
}
...
impl Widget for DragValue<'_> {
    fn ui(self, ui: &mut Ui) -> Response {
        ...
        if ui.memory_mut(|mem| gained_focus(mem, id)) {
        ...
        if ui.memory(|mem| lost_focus(mem, id)) && !ui.input(|i| i.key_pressed(Key::Escape)) {
