use crate::{
    ...
    render::{DrawDataHandles, GizmoDrawData, TransformGizmoRenderPlugin},
};
...
pub use crate::{
    enum_set, math,
    math::{Pos2, Rect},
    EnumSet, Gizmo, GizmoConfig, GizmoDirection, GizmoInteraction, GizmoMode, GizmoOrientation,
    GizmoResult, GizmoVisuals,
};
...
fn draw_gizmos(
    ...
    mut draw_data_assets: ResMut<Assets<GizmoDrawData>>,
    ...
    for (gizmo_uuid, gizmo) in &gizmo_storage.gizmos {
        let draw_data = gizmo.draw(scale_factor);

        let mut bevy_draw_data = GizmoDrawData::default();

        let (GizmoDrawData(asset), is_new_asset) =
        ...
        let viewport = &gizmo.config().viewport_rect;
        ...
        asset
            .vertices
            .extend(draw_data.vertices.into_iter().map(|[x, y]| {
                [
                    ((x - min.x) / size.x) * 2.0 - 1.0,
                    ((y - min.y) / size.y) * 2.0 - 1.0,
                ]
            }));

        asset.colors = draw_data.colors;
        asset.indices = draw_data.indices;
