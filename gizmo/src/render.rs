...
impl Plugin for TransformGizmoRenderPlugin {
    fn build(&self, app: &mut App) {
        load_internal_asset!(app, GIZMO_SHADER_HANDLE, "render.wgsl", Shader::from_wgsl);
...
pub struct DrawDataHandles {
    pub handles: HashMap<Uuid, Handle<GizmoDrawData>>,
}
...
pub struct GizmoDrawData(pub super::DrawData);
...
fn queue_transform_gizmos(
    ...
    for (
        ...
        view,
        _render_layers,
        (normal_prepass, depth_prepass, motion_prepas, deferred_prepass),
    ) in &mut views
    {
        ...
        view_key.set(MeshPipelineKey::NORMAL_PREPASS, normal_prepass);
        view_key.set(MeshPipelineKey::DEPTH_PREPASS, depth_prepass);
        view_key.set(MeshPipelineKey::MOTION_VECTOR_PREPASS, motion_prepas);
        view_key.set(MeshPipelineKey::DEFERRED_PREPASS, deferred_prepass);
        ...
        for (entity, handle) in &transform_gizmos {
            ...
            let key = TransformGizmoPipelineKey {
                view_key,
                perspective: true,
            };
