...
impl<'a, 'storage> FnBuilder<'a, 'storage> {
    ...
    pub fn for_input_f32(&mut self, input: Port) -> EmitResult {
        self.for_input_resolve(input, VectorKind::V1)
    }
    pub fn for_input_f32x2(&mut self, input: Port) -> EmitResult {
        self.for_input_resolve(input, VectorKind::V2)
    }
    pub fn for_input_f32x3(&mut self, input: Port) -> EmitResult {
        self.for_input_resolve(input, VectorKind::V3)
    }
    pub fn for_input_f32x4(&mut self, input: Port) -> EmitResult {
    ...
    pub fn function_by_name(&self, name: &str) -> Option<Handle<Function>> {
        self.module
            ...
            .functions
            .iter()
            .find_map(|(handle, f)| (f.name.as_deref() == Some(name)).then_some(handle))
    }
...
impl<'a> StructBuilder<'a> {
    ...
    pub fn append(
        ...
        let span = ty_inner.size(naga::proc::GlobalCtx {
    ...
    pub fn build(self) -> Handle<Type> {
        ...
        module.types.insert(Type { name, inner }, Span::default())
    }
}
