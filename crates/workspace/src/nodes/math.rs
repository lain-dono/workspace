...
use super::util::*;
use naga::{
    BinaryOperator, DerivativeAxis, DerivativeControl, Expression, MathFunction, UnaryOperator,
};
    ...
    pub fn spawn(storage: &mut Storage, op: UnaryOperator) -> Node {
        storage.spawn(format!("{:?}", op), 100.0, |ctx, node| {
            let data = match op {
                ...
                UnaryOperator::LogicalNot => Data::Boolean,
            };
...
impl PreviewBuilder for Derivative {
    ...
    fn output_expr(&self, _: Node, function: &mut FnBuilder, output: Port) -> EmitResult {
        ...
        Ok(function.emit(Expression::Derivative { axis, ctrl, expr }))
    }
}
...
impl PreviewBuilder for Posterize {
    fn output_expr(&self, _: Node, function: &mut FnBuilder, output: Port) -> EmitResult {
        ...
        let one = dst.splat(function, F32(1.0))?;
