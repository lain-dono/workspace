...
impl PreviewBuilder for Flipbook {
    ...
    fn output_expr(&self, _: Node, function: &mut FnBuilder, output: Port) -> EmitResult {
        ...
        let flip_x = F32(if self.flip_x { 1.0 } else { 0.0 });
        let flip_y = F32(if self.flip_y { 1.0 } else { 0.0 });

        let uv = Wrap(function.for_input_f32x2(self.uv)?);
        let tile = Wrap(function.for_input_f32(self.tile)?);
        let width = Wrap(function.for_input_f32(self.width)?);
        let height = Wrap(function.for_input_f32(self.height)?);
