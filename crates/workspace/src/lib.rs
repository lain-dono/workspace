...
impl Storage {
        ...
        F: for<'a> FnOnce(&mut NodeBuilder<'a>, Node) -> B,
    {
        let mut node_builder = NodeBuilder {
            storage: unsafe { &mut *(self as *mut _) },
            inputs: Vec::new(),
            outputs: Vec::new(),
        };
    ...
    fn unlink_impl(
        ...
        links.remove(link).inspect(|removed| {
            log::debug!(
                "unlink {:?} [{:?} {:?}] -> [{:?} {:?}]",
                link,
                removed.min.node,
...
impl Workspace {
    pub fn draw(&mut self, ctx: &egui::Context, panel_rect: egui::Rect) {
        self.dirty = ctx.input(|input| self.input.update(input));

        let mouse_is_over_area = panel_rect.contains(self.input.pointer);

        match self.interaction {
            Interaction::None => {
                if mouse_is_over_area && ctx.input(|input| input.key_pressed(egui::Key::Space)) {
            ...
            Interaction::Panning => {
                self.pan_offset += ctx.input(|input| input.pointer.delta());
                if self.input.mouse[self.pan_button as usize].released {
                    self.interaction = Interaction::None;
                }
            }
            Interaction::BoxSelection { start } => self.box_selection(ctx, panel_rect, start),
            Interaction::LinkCreation { current } => log::debug!("link creation {current:?}"),
        ...
        for (node_key, node) in &mut self.storage.nodes {
            ...
            let selected = selected.then_some(self.selection_outline);
                ...
                match interaction {
                    ...
                    NodeInteraction::LinkStart(mut current) => {
                        if let Interaction::None = self.interaction {
                            ...
                            if port.is_input()
                                && let Some(link) = port.links.iter().next().copied()
                            {
                                let ports = &mut self.storage.ports;
                                let links = &mut self.storage.links;
                                let link = Storage::unlink_impl(ports, links, link);
        ...
        if let Some((key, delta)) = drag_delta {
            ...
            for &node in &self.selection {
                let node = self.storage.nodes.get_mut(node);
                ...
                for &port in node.inputs.iter().chain(node.outputs.iter()) {
                    ...
                    if let Some(layer_id) = port.input_default.as_ref().and_then(|d| d.layer_id) {
                        let transform = egui::emath::TSTransform::from_translation(delta);
        ...
        if let Interaction::LinkCreation { current } = self.interaction {
            ...
            let max = if let Some(port) = snap_port {
                self.storage.ports[port].position
            } else {
                self.input.pointer
            };
    ...
    fn box_selection(&mut self, ctx: &egui::Context, panel_rect: egui::Rect, start: egui::Pos2) {
        let btn = self.selection_button as usize;
        let current = self.input.pointer;

        let rect = egui::Rect::from_min_max(start.min(current), start.max(current));
        let rect = panel_rect.shrink(1.0).intersect(rect);
    ...
    fn node_creation(&mut self, ctx: &egui::Context, position: egui::Pos2, first: bool) {
        let area_position = {
            let egui::Rect { min, mut max } = ctx.input(|input| input.content_rect());
            max = (max - egui::vec2(250.0, 200.0)).max(min);
            position.clamp(min, max)
        };

        let area = egui::Area::new("NodeCreation".into()).order(egui::Order::Foreground);
    ...
    &[
        ...
        Group {
            items: &[
                ...
                ("Not", |s| Unary::spawn(s, UnaryOperator::LogicalNot)),
