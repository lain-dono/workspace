use bevy::{
    ...
    render::{
        ...
        render_resource::{
            BindGroup, BindGroupEntries, BindGroupLayout, Buffer, RenderPipeline, ShaderType,
        },
...
impl Plugin for GizmoPlugin {
    ...
    fn finish(&self, app: &mut App) {
        ...
        app.sub_app_mut(RenderApp)
            .add_render_graph_node::<ViewNodeRunner<GizmoCameraDriver>>(
...
fn extract_lines(mut lines: ResMut<Lines>, device: Res<RenderDevice>, queue: Res<RenderQueue>) {
    lines.vertex.upload(&device, &queue);
    let to_draw = lines.index.upload(&device, &queue);
}
...
impl ViewNode for GizmoCameraDriver {
    type ViewQuery = (
        Read<ViewTarget>,
    ...
    */
    ...
    fn run(
        ...
        (target, depth, offset): QueryItem<Self::ViewQuery>,
        world: &World,
    ) -> Result<(), NodeRunError> {
        let pipeline = world.resource::<LinesPipeline>();
