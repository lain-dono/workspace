use super::component::ProxyPointLight;
...
impl EditorTab for SceneTab {
    ...
    fn ui(
        ...
        ui: &mut egui::Ui,
        entity: Entity,
        (ctx, mapping, query, selection): &mut SystemParamItem<'_, '_, Self::Param>,
    ) {
        ...
        if let Some(texture_id) = target.texture_id {
            ...
            let screen_rect = response.rect;
            {
                ...
                camera_view.translation +=
                    movement * ui.input(|input| input.predicted_dt) * mov_speed;
            }
            ...
            for &(_z, center, index) in &self.zsorting {
                ...
                let entity = &mut ctx.scene_mut().entities[index];
