...
impl<T: bevy::asset::Asset> ComponentEditor for ProxyHandle<T> {
    ...
    fn ui(ui: &mut egui::Ui, style: &Style, registry: &TypeRegistry, reflect: &mut dyn Reflect) {
        ...
        super::editor::component_header(None, ui, style, icon, &name, |ui| {
            let field = reflect.reflect_path_mut("path").unwrap();
            crate::inspector::ui_for_reflect(ui, registry, field);
        });
    }
}
