...
pub trait ComponentEditor {
    ...
    fn ui(ui: &mut Ui, style: &Style, registry: &TypeRegistry, reflect: &mut dyn Reflect) {
        let (icon, name) = Self::desc();
        reflect_component_editor(ui, style, registry, reflect, icon, &name);
    }
}
...
pub struct ReflectComponentEditor {
    skip: fn() -> bool,
    ui: fn(&mut Ui, style: &Style, registry: &TypeRegistry, &mut dyn Reflect),
}
...
impl ReflectComponentEditor {
    ...
    pub fn ui(
        ...
        (self.ui)(ui, style, registry, reflect);
    }
}
...
pub fn reflect_component_editor(
    ...
    if component_header(Some(&mut state), ui, style, icon, name, |_| ()) {
        ...
        Frame::none().inner_margin(margin).show(ui, |ui| {
            crate::inspector::ui_for_reflect(ui, registry, reflect);
        });
    }
...
pub fn component_header(
    ...
    {
        let left_x = egui::lerp(rect.min.x..=rect.max.x, crate::inspector::PERCENT);
