...
use bevy::reflect::{
    serde::ReflectSerializer, Reflect, TypeRegistry, TypeRegistryArc, TypeRegistryInternal,
    TypeUuid,
};
...
impl SceneMapping {
    ...
    fn recursive_propagate(&mut self, scene: &ReflectScene, parent_index: usize) -> Option<()> {
        ...
        {
            if let Some(&index) = self.entity.get(&child.index()) {
...
impl ReflectSceneSpawner {
    ...
    pub fn spawn_sync(
        ...
        let spawned = self.scenes.entry(scene_handle.clone()).or_default();
...
#[derive(Asset, Default, TypeUuid, TypePath, Resource)]
            ...
            for component_id in archetype.components() {
                ...
                if let Some(reflect) = reflect {
                    for (index, entity) in archetype.entities().iter().enumerate() {
                        ...
                        if let Some(component) = reflect.reflect(entity) {
    ...
    pub fn write_to_world(
        ...
        for ReflectEntity { entity, components } in &self.entities {
            ...
            for component in components.iter().map(AsRef::as_ref) {
                ...
                if entity.contains_type_id(type_id) {
                    reflect.apply(&mut entity, component);
                } else {
                    reflect.insert(&mut entity, component);
                }
            }
        }

        for (type_id, entities) in scene_mappings.into_iter() {
            let registration = registry.get(type_id).expect(
            ...
            if let Some(reflect) = registration.data::<ReflectMapEntities>() {
                reflect.map_entities(world, entity_map, &entities);
            }
        }

        let mut mapping = world.get_resource_or_insert_with(SceneMapping::default);
        mapping.update(self);
...
impl<'a, 'de> serde::de::Visitor<'de> for ComponentSeqVisitor<'a> {
        ...
        A: serde::de::SeqAccess<'de>,
    {
        ...
        while let Some(entity) = seq.next_element_seed(UntypedReflectDeserializer::new(registry))? {
