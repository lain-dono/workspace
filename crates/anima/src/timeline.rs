use super::runtime::{BoneClipData, Curve, PlayControl, PlayState};
use super::AnimaEditorContext;
...
impl EditorTab for TimelinePanel {
    type Param = (SRes<Style>, AnimaEditorContext<'static, 'static>);

    fn ui(
        &mut self,
        ui: &mut egui::Ui,
        _entity: Entity,
        (style, editor): &mut SystemParamItem<'_, '_, Self::Param>,
    ) {
        let animation = &mut editor.data.clips[editor.state.current_clip as usize];
        ...
        ui.scope(|ui| {
            ...
            ui.scope(|ui| {
                ...
                let response = ui.allocate_ui(size, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if let Some(control) = play_control(ui, editor.state.state) {
                            editor.state.action(control, animation.max_time());
                        }
                ...
                let shape = Shape::rect_filled(rect, 0.0, style.panel_fill);
                ...
                for i in 0..=count {
                    ...
                    let px = ppi.recip();
                    if i == editor.state.current_time {
                        ui.painter().set(
                            bg_idx,
            ...
            scroll.show(ui, |ui| {
                ...
                for bone in &mut animation.bones {
                    update_keys(&mut self.keys, bone);
                    bone.draw(ui, style, editor.state.current_time, &self.keys);
                }
            });
        });
    }
...
impl BoneClipData {
    ...
    fn draw(&mut self, ui: &mut egui::Ui, style: &Style, current_time: u32, keys: &[u32]) {
        let extra = self.extra_lines();
        if extra == 0 {
            return;
        }

        let id = Id::new(&self.bone).with("_bone");
        let mut state = OpenState::load(ui.ctx(), id, true);

        let lines = 1 + if state.is_open() { extra } else { 0 };
        let max_width = ui.available_size_before_wrap().x;
...

        {
            ...
            let open_icon = if state.is_open() {
        ...
        if state.is_open() {
            ...
            if self.show_rotate() {
                self.rotate.ui(ui, id, start, ROTATE_COLOR, "rotate");
                start.y += LINE_HEIGHT;
            }
            if self.show_translate() {
                self.translate
                    .ui(ui, id, start, TRANSLATE_COLOR, "translate");
                start.y += LINE_HEIGHT;
            }
            if self.show_scale() {
                self.scale.ui(ui, id, start, SCALE_COLOR, "scale");
                start.y += LINE_HEIGHT;
            }
            if self.show_shear() {
                self.shear.ui(ui, id, start, SHEAR_COLOR, "shear");
                start.y += LINE_HEIGHT;
            }
        }
...
impl<T: super::runtime::Lerp> super::runtime::Timeline<T> {
    fn ui(&mut self, ui: &mut Ui, id: Id, start: Pos2, color: Color32, label: &str) {
        let px = ui.ctx().pixels_per_point().recip();
        let id = id.with(label);

        let mut iter = self.iter_mut().peekable();

        while let Some((&current_time, current)) = iter.next() {
            let id = id.with(current_time);

            if let Some(next) = iter.peek().map(|(&time, _)| time) {
                let curr = ROW_WIDTH * current_time as f32;
                let next = ROW_WIDTH * next as f32;

                let a = start + vec2(curr, LINE_HEIGHT - 1.0);
                let b = start + vec2(next, 1.0);
                let b_bot = start + vec2(next, LINE_HEIGHT - 1.0);
                let color = CURVE_COLOR.linear_multiply(CURVE_COLOR_FACTOR);
                match current.curve {
                    Curve::Step => ui.painter().line_segment([a, b_bot], (px, color)),
                    Curve::Linear => ui.painter().line_segment([a, b], (px, color)),
                    Curve::Cubic => {
            ...
            // widget
            let curr_pos = ROW_WIDTH * current_time as f32;
            let rect = egui::Rect {
                min: start + vec2(curr_pos - ROW_WIDTH / 2.0, 0.0),
                max: start + vec2(curr_pos + ROW_WIDTH / 2.0, LINE_HEIGHT),
            };
            ...
            let sx = response.rect.width() / 2.0 - 0.5 - px;
            let bar_rect = response.rect.shrink2(vec2(sx, 0.0));
            ui.painter().rect_filled(bar_rect, 0.0, color);
        }
    }
}

fn update_keys(keys: &mut Vec<u32>, data: &BoneClipData) {
    keys.clear();

    keys.extend(data.rotate.time_iter());
    keys.extend(data.translate.time_iter());
    keys.extend(data.scale.time_iter());
    keys.extend(data.shear.time_iter());
...
impl OpenState {
    pub fn load(ctx: &egui::Context, id: egui::Id, default: bool) -> Self {
        let open = default;
        ctx.data(|data| data.get_temp(id).unwrap_or(Self { open }))
    }
