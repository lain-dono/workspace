use reui::{Color, FillRule, Offset, Path, Solidity, Stroke, Transform, plugin::Recorder};
...
impl<'a> Gizmos<'a> {
    pub fn compose(tx: f32, ty: f32, rotation: f32, sx: f32, sy: f32) -> Transform {
    ...
    pub fn new(recorder: &'a mut Recorder, viewport: Transform, zoom: f32) -> Self {
        Self {
            recorder,
            viewport,
            zoom,
    ...
    pub fn set_origin(&mut self, x: f32, y: f32, rotation: f32) {
        let (tx, ty) = (x * self.zoom, y * self.zoom);
        let (sn, cs) = rotation.sin_cos();
        self.transform = self.viewport * Transform::new(cs, -sn, tx, sn, cs, ty);
    }
