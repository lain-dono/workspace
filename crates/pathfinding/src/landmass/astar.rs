...
impl Ord for NodeRef {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        f32::total_cmp(&self.estimate, &other.estimate)
            .then(f32::total_cmp(&other.estimate, &self.cost))
    }
}
...
fn recover_path_from_node<ProblemType: AStarProblem>(
    ...
    loop {
        ...
        let Some((next_index, action)) = &node.previous_node else {
            break;
        };
...
pub(crate) fn find_path<ProblemType: AStarProblem>(
    ...
    fn try_add_node<ProblemType: AStarProblem>(
        ...
        let best_estimate = best_estimates
    ...
    while let Some(Reverse(current_node_ref)) = open_nodes.pop() {
        ...
        if *best_estimates.get(&current_node.state).unwrap() < current_node_ref.estimate {
            continue;
        }
