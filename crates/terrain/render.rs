    ...
    P: PhaseItem,
{
    ...
    fn render<'w>(
        ...
        let buf = custom_phase_item_buffers.into_inner();

        // Tell the GPU where the vertices are.
        let vtx = buf.vertices.buffer().unwrap().slice(..);
        pass.set_vertex_buffer(0, vtx);

        // Tell the GPU where the indices are.
        let idx = buf.indices.buffer().unwrap().slice(..);
        pass.set_index_buffer(idx, IndexFormat::Uint32);
        ...
        let base_descriptor = RenderPipelineDescriptor {
            ...
            fragment: Some(FragmentState {
                ...
                targets: vec![Some(ColorTargetState {
                    ...
                    format: <TextureFormat as bevy::image::BevyDefault>::bevy_default(),
                    blend: None,
