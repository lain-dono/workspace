...
pub fn rect_set_size_centered(rect: &mut Rect, size: Vec2) {
    *rect = rect_size_centered(*rect, size);
}

pub fn rect_size_centered(mut rect: Rect, size: Vec2) -> Rect {
...
pub fn rect_stroke_box(min: Pos2, max: Pos2, width: f32) -> Rect {
    egui::Rect::from_min_max(min, max).expand(-f32::ceil(width / 2.0))
}
...
fn fade_button_style(style: &mut ButtonsStyle, factor: f32) {
    style.close_tab.color = style.close_tab.color.linear_multiply(factor);
    style.close_tab.active_color = style.close_tab.active_color.linear_multiply(factor);
    style.close_tab.bg_fill = style.close_tab.bg_fill.linear_multiply(factor);
    style.add_tab.color = style.add_tab.color.linear_multiply(factor);
    style.add_tab.active_color = style.add_tab.active_color.linear_multiply(factor);
    style.add_tab.bg_fill = style.add_tab.bg_fill.linear_multiply(factor);
    style.add_tab.border_color = style.add_tab.border_color.linear_multiply(factor);
}
