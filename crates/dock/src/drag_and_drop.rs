use super::{
    dock_area::AllowedSplits,
    style::Style,
    tree::{NodeIndex, Split, SurfaceIndex, TabDestination, TabIndex, TabInsert},
};
use egui::{
...
impl TreeComponent {
    pub fn as_tab_destination(&self) -> TabDestination {
        match *self {
            Self::Surface(surface) => TabDestination::EmptySurface(surface),
            Self::Node(surface, node) => TabDestination::Node(surface, node, TabInsert::Append),
            Self::Tab(surface, node, tab) => {
                TabDestination::Node(surface, node, TabInsert::Insert(tab))
            }
        }
    }
    ...
    pub fn node_address(&self) -> (SurfaceIndex, Option<NodeIndex>) {
        match *self {
            Self::Surface(surface) => (surface, None),
            Self::Node(surface, node) => (surface, Some(node)),
            Self::Tab(surface, node, _tab) => (surface, Some(node)),
        }
    }
    ...
    pub fn surface(&self) -> SurfaceIndex {
        match *self {
            ...
            Self::Node(surface, _node) => surface,
            Self::Tab(surface, _node, _tab) => surface,
        }
    }
...
fn make_overlay_painter(ui: &Ui) -> Painter {
    let layer_id = LayerId::new(Order::Foreground, Id::new("overlay"));
    ui.ctx().layer_painter(layer_id)
}

fn draw_highlight_rect(rect: Rect, ui: &Ui, style: &Style) {
    make_overlay_painter(ui).rect(
...
fn button_ui(
    ...
    if is_mouse_over && !*lock {
        ...
        painter.rect_filled(rect, 0.0, style.overlay.selection.color);
    }
...
impl DragDropState {
    ...
    pub(super) fn resolve_icon_based(
        ...
        for split in [Split::Below, Split::Right, Split::Above, Split::Left] {
            match allowed_splits {
                AllowedSplits::Vertical if !split.is_vertical() => (),
                AllowedSplits::Horizontal if !split.is_horizontal() => (),
                AllowedSplits::None => (),
                _ => {
                    ...
                    if button_ui(
                        ...
                        style,
                        Some(split),
                    ) && let TreeComponent::Node(surface, node) = self.hover.dst
                    {
                        destination =
    ...
    pub(super) fn resolve_traditional(
        ...
        if !windows_allowed && !self.hover.dst.surface().is_main() {
            return None;
        }
        draw_highlight_rect(self.hover.rect, ui, style);
        ...
        let (tab_insertion, overlay_rect) = {
            ...
            let a_pos = pos2(
                inverse_lerp(hover_rect.x_range().into(), pointer.x).unwrap() - 0.5,
                inverse_lerp(hover_rect.y_range().into(), pointer.y).unwrap() - 0.5,
            );
            ...
            if center_drop_rect.contains(a_pos) {
                ...
                let a_pos = match allowed_splits {
                    ...
                    AllowedSplits::Horizontal => pos2(a_pos.x, 0.0),
                    AllowedSplits::Vertical => pos2(0.0, a_pos.y),
        ...
        match final_result {
            ...
            Some(_) => draw_drop_rect(hover_rect.intersect(overlay_rect), ui, style),
            None => (),
        }
    ...
    fn update_lock(&mut self, target_state: LockState, style: &Style, ctx: &Context) {
        match self.locked.as_mut() {
            Some(lock_time) => {
                ...
                let window_hold = if self.hover.dst.surface().is_main() {
                    false
                } else {
                    ctx.request_repaint();
    ...
    pub(super) fn is_locked(&self, style: &Style, ctx: &Context) -> bool {
        self.locked.is_some_and(|lock_time| {
    ...
    fn window_preview_rect(&self, rect: Rect) -> Rect {
        if self.drag.src.surface().is_main() {
...
fn draw_drop_rect(rect: Rect, ui: &Ui, style: &Style) {
    make_overlay_painter(ui).rect_filled(rect, 0.0, style.overlay.selection.color);
}

// Draws a stroked rect describing where a tab will be dropped.
#[inline(always)]
fn draw_window_rect(rect: Rect, ui: &Ui, style: &Style) {
    make_overlay_painter(ui).rect_stroke(rect, 0.0, style.overlay.selection, StrokeKind::Inside);
}
