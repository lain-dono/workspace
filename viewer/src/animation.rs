...
pub fn plugin(app: &mut App) {
    app.add_systems(
        Update,
        (
            handle_inputs.run_if(not(egui_wants_any_input)),
            assign_clips,
        ),
    );
}
...
impl Clips {
    fn new(nodes: Vec<AnimationNodeIndex>) -> Self {
...
fn assign_clips(
    ...
    for (clip_id, clip) in clips.iter() {
        ...
        let Some(ancestor_player) = ancestor_player else {
            warn!("Unexpected animation hierarchy for animation clip {clip_id}; ignoring.");
            continue;
        };

        let Some(clip_handle) = assets.get_id_handle(clip_id) else {
            warn!("Clip {clip_id} wasn't loaded.");
            continue;
        };
...
fn handle_inputs(
    ...
    mut animation_player: Query<(&mut AnimationPlayer, &mut Clips, NameOrEntity)>,
) {
    for (mut player, mut clips, name) in &mut animation_player {
        if keyboard_input.just_pressed(KeyCode::Space) {
            if player.all_paused() {
                info!("resuming animations for {name}");
                player.resume_all();
            } else {
                info!("pausing animation for {name}");
                player.pause_all();
            }
        }
        ...
        if keyboard_input.just_pressed(KeyCode::Enter) {
            info!("switching to new animation for {name}");
            ...
            player.play(clips.current()).repeat();
