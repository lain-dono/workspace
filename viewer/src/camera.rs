...
pub fn plugin(app: &mut App) {
    app.add_systems(
        Update,
        run_camera_controller.run_if(not(egui_wants_any_input)),
    );
}
...
fn run_camera_controller(
    time: Res<Time>,
    mut windows: Query<(&mut CursorOptions, &Window)>,
    ...
    if cursor_grab_change {
        if cursor_grab {
            for (mut cursor_options, window) in &mut windows {
                if window.focused {
            ...
            for (mut cursor_options, _) in &mut windows {
