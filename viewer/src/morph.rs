...
impl fmt::Display for Target {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let (entity, index, weight) = (self.entity, self.index, self.weight);
        match (self.name.as_ref(), self.entity_name.as_ref()) {
            (None, None) => write!(f, "animation{index} of {entity}"),
            (None, Some(this)) => write!(f, "animation{index} of {this}"),
            (Some(target), None) => write!(f, "{target} of {entity}"),
            (Some(target), Some(this)) => write!(f, "{target} of {this}"),
        }?;
        write!(f, ": {weight}")
    }
}
...
impl Target {
    ...
    fn new(
        ...
        let get_name = |index| target_names.and_then(|names| names.get(index));
