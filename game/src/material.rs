...
pub fn setup(
    ...
    let settings = |settings: &mut ImageLoaderSettings| settings.sampler = ImageSampler::nearest();

    let base = asset_server.load_with_settings("palette/base.png", settings);
    let attr = asset_server.load_with_settings("palette/attr.png", settings);
