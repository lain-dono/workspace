...
impl Earcut {
    ...
    fn reset(&mut self, capacity: usize) {
        ...
        self.nodes.push(Node::new(0, [f32::INFINITY; 2])); // dummy node
    }
    ...
    pub fn earcut<N: Index>(
        &mut self,
        data: impl IntoIterator<Item = [f32; 2]>,
        hole_indices: &[N],
...
fn is_ear_hashed<'a>(
    ...
    loop {
        ...
        if ((p.xy[0] >= xy_min[0])
            ...
            && point_in_triangle(a.xy, b.xy, c.xy, p.xy)
            && area(node!(nodes, p.prev_i), p, node!(nodes, p.next_i)) >= 0.0
        {
            return (false, a, c);
        }
        ...
        if ((n.xy[0] >= xy_min[0])
            ...
            && area(node!(nodes, n.prev_i), n, node!(nodes, n.next_i)) >= 0.0
        {
            return (false, a, c);
        }
...
/// interlink polygon nodes in z-order
fn index_curve(nodes: &mut [Node], start_i: NodeIndex, min_x: f32, min_y: f32, inv_size: f32) {
    let mut p_i = start_i;
...
fn sort_linked(nodes: &mut [Node], list_i: NodeIndex) {
...
fn get_leftmost(nodes: &[Node], start_i: NodeIndex) -> (NodeIndex, &Node) {
    let mut p_i = start_i;
...
fn is_valid_diagonal(nodes: &[Node], a: &Node, b: &Node, a_next: &Node, a_prev: &Node) -> bool {
    ...
    (((a_next.i != b.i) && (a_prev.i != b.i)) && !intersects_polygon(nodes, a, b))
        ...
        && ((locally_inside(nodes, a, b) && locally_inside(nodes, b, a) && middle_inside(nodes, a, b))
            ...
            && (area(a_prev, a, b_prev) != 0.0 || area(a, b_prev, b) != 0.0)
            // special zero-length case
            || equals(a, b)
                && area(a_prev, a, a_next) > 0.0
                && area(b_prev, b, b_next) > 0.0)
}
...
fn eliminate_hole(nodes: &mut Vec<Node>, hole_i: NodeIndex, outer_node_i: NodeIndex) -> NodeIndex {
...
fn find_hole_bridge(nodes: &[Node], hole: &Node, outer_node_i: NodeIndex) -> Option<NodeIndex> {
...
fn sector_contains_sector(nodes: &[Node], m: &Node, p: &Node) -> bool {
    area(node!(nodes, m.prev_i), m, node!(nodes, p.prev_i)) < 0.0
        && area(node!(nodes, p.next_i), m, node!(nodes, m.next_i)) < 0.0
}

/// eliminate colinear or duplicate points
fn filter_points(nodes: &mut [Node], start_i: NodeIndex, end_i: Option<NodeIndex>) -> NodeIndex {
    ...
    loop {
        let p_next = node!(nodes, p.next_i);
        if !p.steiner && (equals(p, p_next) || area(node!(nodes, p.prev_i), p, p_next) == 0.0) {
...
fn split_polygon(nodes: &mut Vec<Node>, a_i: NodeIndex, b_i: NodeIndex) -> NodeIndex {
...
fn insert_node(nodes: &mut Vec<Node>, i: u32, xy: [f32; 2], last: Option<NodeIndex>) -> NodeIndex {
...
fn remove_node(nodes: &mut [Node], pl: LinkInfo) -> (NodeIndex, NodeIndex) {
...
pub fn deviation<N: Index>(
    ...
    let data = data.into_iter().collect::<Vec<[f32; 2]>>();
    let has_holes = !hole_indices.is_empty();
...
#[allow(clippy::too_many_arguments)]
fn point_in_triangle(a: [f32; 2], b: [f32; 2], c: [f32; 2], p: [f32; 2]) -> bool {
