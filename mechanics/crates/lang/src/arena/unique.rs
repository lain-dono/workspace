//! The [`Unique`] type and supporting definitions.

use super::{BadHandle, FastIndexSet, Handle, Index, Span};
use core::{fmt, hash, ops};

/// An arena whose elements are guaranteed to be unique.
///
/// A `Unique` holds a set of unique values of type `T`, each with an
/// associated [`Span`]. Inserting a value returns a `Handle<T>`, which can be
/// used to index the `Unique` and obtain shared access to the `T` element.
...
///
/// `Unique` is similar to [`Arena`]: If `Arena` is vector-like,
/// `Unique` is `HashSet`-like.
///
/// [`Arena`]: super::Arena
#[derive(Clone)]
pub struct Unique<T> {
...
impl<T> Unique<T> {
    ...
    pub(crate) fn drain_all(&mut self) -> UniqueDrain<T> {
        UniqueDrain {
...
impl<T: Eq + hash::Hash> Unique<T> {
    ...
    pub fn iter(&self) -> impl DoubleEndedIterator<Item = (Handle<T>, &T)> + ExactSizeIterator {
        self.set
            .iter()
            .enumerate()
            .map(|(index, v)| (unsafe { Handle::from_usize_unchecked(index) }, v))
    }
    ...
    pub fn push(&mut self, value: T) -> Handle<T> {
        self.insert(value, Span::UNDEFINED)
    }
...
impl<T: fmt::Debug + Eq + hash::Hash> fmt::Debug for Unique<T> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        f.debug_map().entries(self.iter()).finish()
    }
}

impl<T> ops::Index<Handle<T>> for Unique<T> {
    type Output = T;
...
#[cfg(feature = "serde")]
impl<T: Eq + hash::Hash + serde::Serialize> serde::Serialize for Unique<T> {
...
#[cfg(feature = "serde")]
impl<'de, T: Eq + hash::Hash + serde::Deserialize<'de>> serde::Deserialize<'de> for Unique<T> {
