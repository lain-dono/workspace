...
impl<G: Graph> BitSet<G> {
    pub fn new(num_nodes: usize, bits_per_node: usize) -> Self {
    ...
    fn index(&self, node: G::Node) -> usize {
        node.into() * words(self.bits_per_node)
    }
    ...
    pub fn insert_bits_from_node(&mut self, source_node: G::Node, target_node: G::Node) -> bool {
        if source_node == target_node {
            false
        } else {
            let words_per_node = words(self.bits_per_node);
            let source_start = source_node.into() * words_per_node;
            let target_start = target_node.into() * words_per_node;
...
impl BitBuf {
    ...
    pub fn set(&mut self, index: usize) -> bool {
        self.mutate(index, |value, mask| value | mask)
    }
    ...
    fn mutate(&mut self, index: usize, op: impl FnOnce(Word, Word) -> Word) -> bool {
        let (word, bit) = words_bits(index);
        let old_value = self.words[word];
...
fn words_bits(x: usize) -> (usize, usize) {
    (x / Word::BITS as usize, x % Word::BITS as usize)
}
